 ...
 alter table  processreqs  ADD COLUMN mkprdpurordsId numeric(22,0)  DEFAULT 0  not null;
 ...
 --- and rerun 'Create table,view,screen'  again
 --- ----------------------------------------------
 --- alter table processreqs DROP COLUMN mkprdpurordsid CASCADE;
 ...
 --- select pobject_code_scr,pobject_code_sfd,
							...
							---	case screenfield_hideflg when 1 then '' else '表示有' end display,
							---   case screenfield_indisp when 1 then '必須' else '' end inquire from r_screenfields 
 ---- where  pobject_code_sfd = 'processreq_mkprdpurordsid'
 --- update screenfields set expiredate = '2000-01-01',remark = 'auto delete because of DROP COLUMN mkprdpurordsid' 
 ---        ,updated_at = current_date  ,selection = '0'
 ---        where id in  (select id from r_screenfields where  pobject_code_sfd = 'processreq_mkprdpurordsid' 
 ---        and  pobject_code_scr like '%_processreqs'  and screenfield_selection = '1');
